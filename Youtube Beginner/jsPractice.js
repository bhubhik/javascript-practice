...
const smallString = (str) =>
  str.length % 2 == 0 ? str.slice(0, str.length / 2) : 'Give  a even string';
console.log(smallString('Threejelloe'));

// Exercise 3 Program to concatenate two strings excet first characters

const addedChar = (strA, strB) => strA.slice(1, 4) + strB.slice(1);
...
const integers = (arr) => arr.filter((num) => num % 2 === 0).length;
...
const passedNum = (arr) => Math.max(...arr.filter((num) => num % 2 === 0));
...
const sendString = (str) => str.replace(/[0-9]/g, '$');
console.log(sendString('Anubha6b'));
console.log(sendString('8pi67y'));

const changeName = (str) => str.replace(/anubhab/gi, 'Bhubhik');
console.log(
  changeName('My name is Anubhab, You are anuBHaB, Thank you anubhab')
);
...
const RandomHexColor = () =>
  '#' + Array.from({ length: 6 }).map(randomColor).join('');
...
const vowelStr = (str) =>
  str.split('').filter((char) => letters.indexOf(char) > -1).length;
...
console.log(
  getUniqueChars('the quick brown fox jumps over the lazy dog').length
);
//or
...
// Exercise  : Program to find the first not repeated character
const notRepeat = (str) =>
  str
    .split('')
    .filter(
...
console.log(strReverse('anubhab'));

const numbers = [2, 5, 900, 54, 6];
